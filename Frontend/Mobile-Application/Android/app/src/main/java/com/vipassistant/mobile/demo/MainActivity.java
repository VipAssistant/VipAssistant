...
public class MainActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/* set-up SocialDistancingHandler for periodic Map refreshing */
		this.socialDistancingHandler.postDelayed(new Runnable() {
			@Override
			public void run() {
				if (socialDistancingActive) {
					scanDevicesAndAddToList();
					checkSocialStatus();
				}
	...
	private void checkSocialStatus() {
		Pair<Boolean, String> result = RequestService.checkUserSocialDistanceSet();
		if (result.first) {
			AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(MainActivity.this);
			dialogBuilder.setIcon(R.drawable.covid_dialog);
			dialogBuilder.setTitle("Sorry to Bother You But...");
			dialogBuilder.setMessage(result.second);
			dialogBuilder.setPositiveButton("OK...", new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
			...
			Notification notify = new Notification.Builder
					(getApplicationContext()).setContentTitle("Covid-19 Alert")
					.setContentText(result.second).
							setContentTitle(result.second).setSmallIcon(R.drawable.covid_dialog).build();
	...
	private void scanDevicesAndAddToList() {
		...
		BroadcastReceiver mReceiver = new BroadcastReceiver() {
			public void onReceive(Context context, Intent intent) {
				...
				if (BluetoothDevice.ACTION_FOUND.equals(action)) {
					...
					Pair<Boolean, String> opResult = RequestService.addSocialDistancingPeople(device.getAddress());
					if (!opResult.first) {
						Log.w(String.valueOf(Log.ERROR), "Could not add new social distancing device to backend!");
					}
					String notifMsg = "There seems to be a new VipAssistant user in your social distance radius," +
							" we've saved his address to your nearby person list for precaution.";
					NotificationManager notif = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
					Notification notify = new Notification.Builder
