...
public class VipNavigationFragment extends Fragment implements OnMapsceneRequestCompletedListener, OnRoutingQueryCompletedListener, OnPrecacheOperationCompletedListener {
	private View root;
	private VipNavigationViewModel vipNavigationViewModel;
	...
	* which will trigger the second and the third demos. */
	...
	public View onCreateView(@NonNull LayoutInflater inflater,
		...
		vipNavigationViewModel = ViewModelProviders.of(getActivity()).get(VipNavigationViewModel.class);
		root = inflater.inflate(R.layout.fragment_vipnavigation, container, false);
	...
	private void updateMapPeriodically() {
		...
		if (vipNavigationViewModel.getCachingActivated() && cachingTimeout >= 3) {
...
//			alertDialogBuilder.setPositiveButton("Find Me A ...!", new DialogInterface.OnClickListener() {
	...
	public void onPrecacheOperationCompleted(PrecacheOperationResult precacheOperationResult) {
		if (vipNavigationViewModel.getCachingToastActivated()) {
			String toastMessage;
	...
	private class MarkerClickListenerImpl implements OnMarkerClickListener {
		public void onMarkerClick(Marker marker) {
			if (marker.getTitle() != "You Are Here!") {
				Context context = VipNavigationFragment.this.getActivity();
				AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(getActivity());
				dialogBuilder.setIcon(android.R.drawable.ic_dialog_map);
